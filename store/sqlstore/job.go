...
returning *`, map[string]interface{}{
	...
	if err != nil {
		return nil, model.NewInternalError("store.sql_job.save.app_error", "id="+job.Id+", "+err.Error())
	}
...
func (jss SqlJobStore) UpdateOptimistically(job *model.Job, currentStatus string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if sqlResult, err := jss.GetMaster().Exec(
			...
			result.Err = model.NewInternalError("store.sql_job.update.app_error", "id="+job.Id+", "+err.Error())
		} else {
...
func (jss SqlJobStore) UpdateStatus(id string, status string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := jss.GetMaster().UpdateColumns(func(col *gorp.ColumnMap) bool {
			...
			result.Err = model.NewInternalError("store.sql_job.update.app_error", "id="+id+", "+err.Error())
		}
	...
	return store.Do(func(result *store.StoreResult) {
		...
		if err := jss.GetReplica().SelectOne(&status,
			...
			if err == sql.ErrNoRows {
				result.Err = model.NewNotFoundError("store.sql_job.get.app_error", "Id="+id+", "+err.Error())
			} else {
				result.Err = model.NewInternalError("store.sql_job.get.app_error", "Id="+id+", "+err.Error())
			}
		} else {
			result.Data = status
		}
...
func (jss SqlJobStore) GetAllPage(offset int, limit int) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := jss.GetReplica().Select(&statuses,
			...
			result.Err = model.NewInternalError("store.sql_job.get_all.app_error", err.Error())
		} else {
...
func (jss SqlJobStore) GetAllByType(jobType string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := jss.GetReplica().Select(&statuses,
			...
			result.Err = model.NewInternalError("store.sql_job.get_all.app_error", "Type="+jobType+", "+err.Error())
		} else {
			result.Data = statuses
		}
	})
...
func (jss SqlJobStore) GetAllByStatus(status string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if _, err := jss.GetReplica().Select(&statuses,
			...
			result.Err = model.NewInternalError("store.sql_job.get_all.app_error", "Status="+status+", "+err.Error())
		} else {
			result.Data = statuses
		}
	})
...
func (jss SqlJobStore) GetNewestJobByStatusAndType(status string, jobType string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if err := jss.GetReplica().SelectOne(&job,
			...
			result.Err = model.NewInternalError("store.sql_job.get_newest_job_by_status_and_type.app_error", "Status="+status+", "+err.Error())
		} else {
			result.Data = job
		}
	})
...
func (jss SqlJobStore) GetCountByStatusAndType(status string, jobType string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if count, err := jss.GetReplica().SelectInt(`SELECT
			...
			result.Err = model.NewInternalError("store.sql_job.get_count_by_status_and_type.app_error", "Status="+status+", "+err.Error())
		} else {
			result.Data = count
		}
	})
...
func (jss SqlJobStore) Delete(id string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if _, err := jss.GetMaster().Exec(
			...
			WHERE
				id = :Id`, map[string]interface{}{"Id": id}); err != nil {
			result.Err = model.NewInternalError("store.sql_job.delete.app_error", "id="+id+", "+err.Error())
		} else {
			result.Data = id
		}
	})
